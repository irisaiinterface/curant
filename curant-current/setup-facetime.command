...
if system_profiler SPAudioDataType 2>/dev/null | grep -q "Curant Call Output"; then
    ...
    echo ""
    echo "======================================================"
    echo " REQUIRED manual step: create the Multi-Output audio device."
    echo " Curant CANNOT hear callers without this, and two of the"
    echo " settings are not guessable -- both were found the hard way"
    echo " on real calls, so please follow them exactly."
    echo ""
    echo " 1. Opening Audio MIDI Setup..."
    open -a "Audio MIDI Setup"
    echo " 2. Click '+' in the bottom-left -> 'Create Multi-Output Device'"
    echo " 3. Check BOTH devices in the list on the right:"
    echo "      - BlackHole 16ch"
    echo "      - your Mac's normal speakers"
    echo " 4. Set 'Primary Device' at the top to:  BlackHole 16ch"
    echo "      NOT your speakers. The primary device's volume scales the"
    echo "      whole group, so with speakers as primary a real call"
    echo "      measured RMS 3.5 (unusable); with BlackHole 16ch as"
    echo "      primary the identical test measured 2144 -- 600x more."
    echo "      BlackHole also has a fixed internal 48kHz clock, so it"
    echo "      cannot drift the way a Bluetooth or speaker clock can."
    echo " 5. Confirm 'Sample Rate' reads 48.0 kHz."
    echo "      A mismatch here is silent and total: a Bluetooth primary"
    echo "      at 44.1kHz against BlackHole at 48kHz produced EXACTLY"
    echo "      0.0 RMS -- days were lost to this before it was found."
    echo " 6. Tick 'Drift Correction' for every device EXCEPT the primary."
    echo " 7. Do NOT add Bluetooth headphones to this group. They"
    echo "      reconnect at 44.1kHz and silently break capture."
    echo " 8. Rename it (right-click -> Rename) to exactly: Curant Call Output"
    echo "======================================================"
    echo ""
    read -r -p "Press Enter once you've created and named it... " _
fi
