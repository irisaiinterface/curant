...
- **Dean's shell access (Aug 2026)** — real shell command execution, deliberately scoped to Home only and Dean only (Cloud's multi-tenant architecture already treats arbitrary command execution as unsafe, the same reasoning that keeps stdio MCP servers off Cloud). Runs sandboxed to a dedicated working directory with a hard-enforced guard against escaping it. Three safety modes exist — confirm-every-time (default), a pre-approved read-only allowlist, or fully autonomous — and the mode is always the customer's own explicit choice, never assumed by Dean. Verified functionally against all three modes plus a workspace-escape attempt, in an isolated test environment — **not yet exercised in a real customer conversation**, which is the more meaningful test for a live security surface like this one.
...
- **Voice-cap hard-stop and Dean's shell access are both real, working code that has never touched real infrastructure.** The voice-cap enforcer's mechanism is confirmed against Vapi's own docs but not against a real call through this specific Telnyx setup; shell access is verified functionally in an isolated test sandbox but not in an actual customer conversation — the more consequential test for a live security surface like shell access specifically.
