...
- **MCP tool connections** — a curated registry, most entries pre-verified official servers (Linear, GitHub, Atlassian/Jira+Confluence+Bitbucket, Figma, HubSpot, Postman, Slack, Notion, Sentry, Square, Descript). Three entries are deliberately **unofficial, community-built servers**, included at explicit request rather than excluded — every entry is tagged official/unofficial and shown with a visible badge everywhere connections are listed, so the customer sees exactly what they're connecting to rather than Curant silently vouching for something unverified: TestRail (Home-only; no official TestRail MCP server exists as of Aug 2026), and Canvas + Schoology (both Home-only, both education-focused). Schoology specifically carries a stronger warning than the others: it has no public API at all, so every option (including this one) works via browser-cookie capture or automated SSO login, not a sanctioned access method — flagged plainly in its own description, not softened. **Note on Bitbucket:** its tools on the shared Atlassian server currently authenticate via API token only — OAuth is on Atlassian's own roadmap but not live yet, unlike Jira/Confluence. Home connects via a CLI command (OAuth for the official servers, manual local setup for the unofficial stdio ones, which need local installation with the customer's own credentials that Curant never touches or stores); Cloud connects via the dashboard with a real, standards-based OAuth flow (metadata discovery + Dynamic Client Registration + PKCE) split across two web requests, since there's no "customer's own machine" to run a local callback server on. Cloud's registry is HTTP/SSE-only by necessity — it genuinely cannot offer Canvas, Schoology, or TestRail, since all three need a locally-run process, which has no equivalent on a shared server. **Honestly flagged:** Cloud's OAuth flow hasn't been tested against a real provider yet — no live app is registered with any vendor and no browser exists in the build environment to complete an interactive login; confirm against a real account before trusting it with a real customer.
- **All third-party OAuth tokens are now encrypted at rest** (Aug 2026 fix) — MCP connector tokens and the new Microsoft Graph tokens (Outlook/Teams, Cloud-only) were being stored in plaintext, the same class of gap already fixed once before for customer Anthropic API keys. Same Fernet encryption now applied consistently across all three.
...
**Cloud specifically:**
- **Login is a two-step SMS code (Aug 2026), not an email address.** Until the audit, Cloud's login accepted an email address and nothing else — there was no password column in the schema at all, so knowing a customer's email was equivalent to being them. Login now texts a single-use 6-digit code to the customer's own verified mobile: hashed at rest, 10-minute expiry, 5-attempt cap, previous codes invalidated on reissue, and responses deliberately identical whether or not the account exists so the page can't be used to enumerate customers. SMS is chosen over an emailed link because the mobile is already the product's identity anchor — but it is honestly a moderate factor, not a strong one (SIM swap and SS7 are real), and is worth revisiting before this handles anything more sensitive.
- **Inbound SMS/voice routing bug fixed at the same time.** Provisioning was writing Curant's own assigned DID into `phone_routing`, while lookups use the *sender's* number — meaning no inbound text or call could ever match a customer. The customer's own mobile is now collected at signup and used for routing, which is both the working key and the safer one.
...
- **Dean's shell containment is implemented but unproven on real hardware.** Commands now run under macOS `sandbox-exec`: writes denied outside the workspace, reads denied on credential stores (`~/.ssh`, `~/.aws`, Keychains, browser profiles, shell history, Curant's own config), outbound network denied. The profile has not been exercised on an actual Mac from the build environment (which is Linux), so it is applied defensively — if the sandbox is unavailable or the profile is rejected, the command still runs but the output carries a visible warning rather than silently pretending containment applied. Verify on a real Mac.
- **A full security audit (6 Aug 2026) found 15 issues, 4 critical. All 15 are now addressed** — see `SECURITY_AUDIT_2026-08-06.md` for the full report and the honest status of each. Fixes were verified by re-running the original exploits and by functional testing, not by inspection. Two carry standing caveats: the macOS sandbox profile is untested on real hardware, and indirect prompt injection is mitigated rather than solved — it is inherent to giving a language model real tools, and the defence is layered (explicit warnings in tool descriptions, confirmation enforced in code, containment and network-deny under the shell), not a single control.
