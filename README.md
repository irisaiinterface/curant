...
- **A full security audit (6 Aug 2026) found 15 issues, 4 critical. All 4 criticals and every High are now fixed** — see `SECURITY_AUDIT_2026-08-06.md` for the full report, including what remains open (SSRF on `browse_page`, `FLASK_DEBUG` reachability, proxy-aware rate limiting, and the broader indirect-prompt-injection posture). The fixes were verified by re-running the original exploits, not by inspection.
