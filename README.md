...
- **MCP tool connections** — a curated registry, most entries pre-verified official servers (Linear, GitHub, Atlassian/Jira+Confluence+Bitbucket, Figma, HubSpot, Slack, Notion, Sentry, Square, Descript). Two entries (Canvas, Schoology — both Home-only, both education-focused) are deliberately **unofficial, community-built servers**, included at explicit request rather than excluded — every entry is tagged official/unofficial and shown with a visible badge everywhere connections are listed, so the customer sees exactly what they're connecting to rather than Curant silently vouching for something unverified. Schoology specifically carries a stronger warning: it has no public API at all, so every option (including this one) works via browser-cookie capture or automated SSO login, not a sanctioned access method — flagged plainly in its own description, not softened. Home connects via a CLI command (OAuth for the official servers, manual local setup for the two unofficial ones — Canvas and Schoology both need local installation with the customer's own credentials, which Curant never touches or stores); Cloud connects via the dashboard with a real, standards-based OAuth flow (metadata discovery + Dynamic Client Registration + PKCE) split across two web requests, since there's no "customer's own machine" to run a local callback server on. Cloud's registry is HTTP/SSE-only by necessity — it genuinely cannot offer Canvas or Schoology, since both need a locally-run process, which has no equivalent on a shared server. **Honestly flagged:** Cloud's OAuth flow hasn't been tested against a real provider yet — no live app is registered with any vendor and no browser exists in the build environment to complete an interactive login; confirm against a real account before trusting it with a real customer.
